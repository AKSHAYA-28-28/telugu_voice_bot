...
def fetch(
    ...
    if str(source).startswith("http:") or str(source).startswith("https:"):
        ...
        MSG = f"Fetch {filename}: Copying local file from {str(sourcepath)}."
        logger.info(MSG)
        _missing_ok_unlink(destination)
        shutil.copy2(sourcepath, destination)  # ✅ Safe for Windows
    else:
        ...
        # Huggingface hub downloads to etag filename, copy to the expected one:
